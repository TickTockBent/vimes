...
- **Transitional core — each field named WITH the move that retires it,**
  ...
  declaration), `manualReviewRequired` (→ workflow data — **amended
  2026-08-10, slice-12 F1:** NOT Move 3 after all; retiring it means
  activating the bounded loop (`max_traversals`/`on_exhausted`), which is
  behaviour-shaping and a fixture hazard, so it rides the BOUNDED-LOOP
  ACTIVATION move with its own D-record. Move 3 kept the flag rule in the
  transitional vocabulary module, F5),
