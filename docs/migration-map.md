...
from `packages/core`.
*Status:* **DONE 2026-08-10** (slice 12, `d01a03b`/`0d2f6ca`/`0ab7b44`):
adjudicator `extensions/proposeMove.ts` + 1458-case parity proof (U1);
shipped manifest asset + boot fail-fast + writer flip + pinned refs +
stage-edges wire-stable via frozen presentation order (U2); table and
machine deleted, `taskStateMachine.ts` reduced to the transitional
vocabulary module, differential + parity re-pointed at frozen images of
the deleted table (U3). Exit gate met in full; grep gate empty across all
three packages. Not yet deployed to the live daemon (clearance pending).

**Move 4 — the tasks extension leaves the engine.** *(ADDED 2026-08-11,
Wes-agreed. The map previously said "everything after move 3 is
per-declaration," which left q29's default with no move to land in: creating
`packages/ext-tasks/` is not a per-declaration move, and the gap was found
while sequencing the mateclaw boundary ratification.)*
Relocate the tasks extension's remaining tenant-specific code out of
`packages/core` and `packages/daemon` into its own workspace package, and
**make the Tier-1 import boundary mechanical in the same unit** — this move's
skeleton owns q29's amended enforcement in full: the extension-host interface
as its own package, `@vimes/core` **absent from `packages/ext-tasks`'s
dependencies** so an internals import fails resolution rather than
type-checking, and a lint rule wired into `ci-gate.sh` as the backstop for
path aliases and transitive re-exports.
*Exit gate:* fixture replays byte-identically; the suite is green; and the
boundary is proven **by sabotage** — add an `@vimes/core` internals import to
`packages/ext-tasks`, confirm it FAILS (resolution first, lint second), then
revert. A boundary that has never been observed refusing anything is not a
boundary.
*Why it is its own move and not folded into a per-declaration one:* it moves
code without moving behaviour, so it is exactly the kind of step the fixture
checks for free — and every per-declaration move that lands before it adds
imports written against the weaker boundary, which is why it wants to be
early in the tail rather than the tidy-up at the end.
...
29. **Does `packages/core/src/tasks/` die into its own package?** **DEFAULT
    ...
    > **AMENDED 2026-08-11 (mateclaw-decompose §2.1.1; Wes-ratified same day).
    > The default stands; its stated rationale does not.** "Enforced by the
    > compiler instead of by review" is **false as written for a TypeScript
    > workspace** — a package boundary does not stop `packages/ext-tasks/`
    > from importing `@vimes/core`; it resolves and compiles. Verified the
    > same day: the repo has **no import-boundary enforcement of any kind**
    > (no `no-restricted-imports`, no dependency-cruiser, nothing in
    > `ci-gate.sh`), so the one property that keeps #15 from eroding silently
    > is today held by review alone — precisely the outcome this default was
    > written to avoid.
    >
    > The mechanism that actually delivers it is mateclaw's, whose plugin
    > JARs compile against `mateclaw-plugin-api` *only* because the server
    > classes are not on the classpath at all. Its three-part TS equivalent,
    > **all three required together**: (a) the extension-host interface is its
    > own workspace package, not a subdirectory of core; (b) `@vimes/core` is
    > **absent from `packages/ext-tasks`'s dependencies**, so an internals
    > import fails resolution rather than type-checking; (c) a lint rule as
    > the backstop for path aliases and transitive re-exports, run in
    > `ci-gate.sh` so it fails a gate rather than a reading.
    >
    > **Timing is the whole point:** this is cheap in the move that *creates*
    > `packages/ext-tasks/` and expensive in every move after it, because
    > each subsequent move adds imports written against the weaker boundary.
    > **Settle it before the next migration move**, as part of that move's
    > skeleton — not as a follow-up unit.
