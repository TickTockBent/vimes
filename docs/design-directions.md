...
> **⚠ SUPERSEDED 2026-08-11 by D70 (signed 2026-08-05) — read this entry as
> history, not as a plan.** This was written in the bespoke-orchestration era,
> when the task machine WAS VIMES. Under D70 the engine is a pure session engine
> and *all workflow is extension content*, so **the board cannot be the home
> surface**: a kanban landing page would encode one tenant's workflow as the
> engine's idea of what work looks like, which is what D70 forbids and #16 makes
> grep-assertable. Book Genesis has chapters, not a backlog.
>
> **What replaces it:** the session TREE is home (E2 vocabulary, tenant-blind),
> the board becomes a `[[panes]]` contribution of the tasks extension, and
> workflow state reaches tree nodes as `[[overlays]]` (extension-model §2.5) —
> declared decorations the engine stores and fans out but never interprets.
>
> **What SURVIVES intact and is still the plan:** the panel substrate itself —
> panel host, layout-aware hash, focus model, close affordance — plus this
> entry's real insight, that *"only WHAT fills the slots changes."* That
> sentence turned out to be truer than it knew: under D70 what fills the slots
> is declared by extensions rather than chosen by us. The closing "Wes's call"
> below was ANSWERED by D70; do not re-open it as a live question (the
> 2026-08-11 design pass did, and `ui-face-pass.md` §1-RESOLVED records why that
> was a misread).
...
## Clacks overhead codes on the messaging protocol (Wes, 2026-08-12) — the easter egg with honest semantics

Born from the push-channel test during slice 13: the test ping went through
the tower deliberately unlogged, which is literally clacks overhead `N` ("do
not log the message"). Wes: "We should absolutely encode Vimes messaging
protocol as clacks style messages. Only someone who looks AND loves
discworld will ever notice but it's fun. Maybe."

**The rule that makes it worth doing: every code maps to REAL semantics the
system already has — flavor names for true facts, never decoration that
lies.** Candidate mapping, from what exists today:

- **G** ("send the message on") — normal delivery: fan-out to every stored
  subscription (`pushPipeline` behavior today).
- **N** ("do not log the message") — out-of-log delivery: the test-ping
  class (`scratchpad/push-test.mjs` is the accidental first instance —
  nothing evented, nothing recorded). Any future diagnostic ping wears it.
- **U** ("turn the message around at the end of the line") — a delivery
  that requests acknowledgment; maps to the D9/D83 ack model (seen/handled)
  when the phone-attention surface (D78) gets its unit.
- **GNU <name>** — the memorial convention stays what it is everywhere.

**Where it lives:** an `overhead` field inside the web-push payload JSON —
invisible in the OS notification, visible to anyone who inspects what the
service worker received. Zero behavioral cost; only lookers notice. Possibly
also as comment vocabulary on `urgencyForAttentionReason` and friends.

**Status: PARKED (Wes's own "Maybe").** Natural slot: the D78
phone-attention unit in the D70-face era — it touches exactly this payload,
so the egg rides a real unit instead of earning its own. Constraint check:
engine-neutrality (#16) is untouched — clacks codes are the DAEMON's
delivery vocabulary, not a tenant's workflow word.

GNU Terry Pratchett.

**Addendum (Wes, same day):** if a protocol guard ever exists that checks
messages are properly ordered, it is named **Carrot** — the officer who
takes the rules completely literally, enforces them on everyone including
his superiors, and is never once wrong to. (An ordering/sequencing validator
on the messaging protocol would sit naturally in the vimes-doctor analyzer
family when the validator work stream lands.)

**Addendum 2 (Wes, same day):** a chaos-engineering extension, if one ever
exists, is **Nobby** (Wes's spelling: Knobby) — it commits petty crimes
against the system on purpose — **with Fred** as the component that keeps
him from making too much trouble: the blast-radius governor / abort
controller every chaos rig needs anyway. The pairing is real architecture
wearing a joke: chaos injection without a Fred is just an outage. As an
extension it is post-engine D70 territory, and it would exercise the same
declared-capability walls (D67 trust model) any tenant does — chaos that
must ASK for the surfaces it may disturb.

**Addendum 3 — the Watch-voice doctrine seed (Wes, same day: "if we get
there, it will be a good day").** The naming jokes share one real design
principle underneath: VIMES already prefers honest sentences over error
codes — `describeRejectionReason` renders the machine's refusals as prose an
operator can act on, dispatch outcomes get tone-classified sentences, the
D33/D9 comments narrate intent. The Watch voice is that same principle with
its hat on: operational messages written as actor + intent + disposition +
why ("Fred has intercepted Nobby's attempt to restart [service] during
[peak window], with an apology. The restart has been postponed."). It is a
BETTER log line by serious observability standards — the apology is
load-bearing, because it means the system knew it was asking for something
disruptive. Seed, not commitment: when notification/log tone gets designed
(the D78 phone surface, the UI doctrine doc's copy rules, or the eventual
chaos governor), check it against this before defaulting to terse
machine-ese. Pairs with the push-notification-tone memory (Wes screenshots
the pings; make them funny, keep them true).

## Cache-aware dispatch policy (parked, 2026-08-12) — extension/workflow territory, NOT engine

Out of a caching exploration with Wes (same day slice 14 opened). Two ideas
survived contact with how prompt caching actually works (content-prefix-
addressed — a cache hit and "the model has read this" are the same fact, so
warmth and contamination cannot be separated):

1. **The warm fixer.** A failed review routes the FIX back to the
   still-live implementer session (resume), which re-hits its own cached
   conversation — allowed by the orchestration skill ("new or resumed"),
   cheap, and legitimate because an implementer fixing its own unit needs
   no independence. Review itself stays blind-and-cold forever: a reviewer
   warmed on the implementer's trail is anchoring wearing a cost
   optimization (the S11-A4 lesson generalized — the implementer's account
   of its work is a fake of the work).
2. **Prefix-disciplined payloads.** Share the GROUND TRUTH, never the
   trail: same agent type (identical system prompt), dispatch prompts that
   open with a byte-identical block (work order, diff, design refs) and
   diverge only at the role tail — the shared prefix caches across
   implementer and reviewer within TTL. Requires inlining shared material
   at the head of the prompt rather than having agents Read it
   (tool-result position kills cross-agent sharing). Pays proportionally
   to payload size; noise for small WOs, real for diff-heavy reviews.

**The classification (Wes): this is dispatch POLICY — E1-d's extension
half.** The engine already records everything such a policy needs (session
identity, resume, dispatch-as-proposal, completion events); deciding WHO
gets dispatched WHEN with WHAT payload shape is workflow content. When an
orchestration workflow extension exists (the software-orchestration lineage
as a VIMES tenant), this is a design input for its dispatcher — not an
engine feature, ever.

*Addendum (Wes, same conversation): the general cold-start problem —
subagents re-deriving repo context every dispatch — still feels like it has
unexploited tricks beyond these two. Parked as a RESEARCH SPIKE, not a
design: candidates when someone picks it up include prefix-stable context
